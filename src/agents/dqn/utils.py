...
class Logger:
    def __init__(self,log_dir = None):
        ...
        if log_dir is None:
            # 这是一个兜底策略，最好是由 dqn.py 传入具体路径
            log_dir = os.path.join(os.getcwd(), 'checkpoints', 'tensorboard_logs_default')

        # 确保目录存在
        os.makedirs(log_dir, exist_ok=True)
        # 初始化 SummaryWriter 并指定路径
        self.writer = SummaryWriter(log_dir=log_dir)
        print(f"[Logger] TensorBoard events will be saved to: {log_dir}")
    ...
    def add_scalar(self, name, data, timestep):
        ...
        self.writer.add_scalar(name, data, timestep)
